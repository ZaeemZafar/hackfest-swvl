...
public extension Date {
    ...
    */
    ...
    func jfToStringWithRelativeTime() -> String {
        if self.compare(.isToday) {
            // return time string
            return self.toString(format: DateFormatType.custom("HH:mm"))
        } else {
            let years = Int(Date().timeIntervalSince(self) / Date.yearInSeconds)
            let days = Int(Date().timeIntervalSince(self) / Date.dayInSeconds)
