...
class PDChartAxesComponentDataItem: NSObject {
    ...
    //optional default
    var showAxes: Bool = false // Default was true to show axis but i made it false
    ...
    var xAxesLeftMargin: CGFloat = 0                   //坐标系左边margin
    var xAxesRightMargin: CGFloat = 100                  //坐标系右边margin
...
class PDChartAxesComponent: NSObject {
    ...
    func getXAxesDegreeTipLabel(tipText: String, center: CGPoint, size: CGSize, fontSize: CGFloat) -> UILabel {
        return self.getAxesDegreeTipLabel(tipText: tipText, center: center, size: size, fontSize: fontSize, textAlignment: NSTextAlignment.center, textColor: dataItem.axesTipColor)
    }
    
    func getYAxesDegreeTipLabel(tipText: String, center: CGPoint, size: CGSize, fontSize: CGFloat) -> UILabel {
        return self.getAxesDegreeTipLabel(tipText: tipText, center: center, size: size, fontSize: fontSize, textAlignment: NSTextAlignment.right, textColor: dataItem.axesTipColor)
    }
    
    func strokeAxes(context: CGContext?) {
        let xAxesWidth: CGFloat = self.getXAxesWidth()
        let yAxesHeight: CGFloat = self.getYAxesHeight()
        let basePoint: CGPoint = self.getBasePoint()
        ...
        if dataItem.showAxes {
            ...
            if (dataItem.xAxesDegreeTexts != nil) {
                for i in 0..<dataItem.xAxesDegreeTexts!.count {
                    ...
                    let label: UILabel = self.getXAxesDegreeTipLabel(tipText: dataItem.xAxesDegreeTexts![i], center: center, size: size, fontSize: dataItem.degreeTipFontSize)
                ...
                for i in 0..<xDegreeNum {
                    let size: CGSize = CGSize(width: xDegreeInterval - dataItem.degreeTipMarginHorizon * 2, height: dataItem.degreeTipFontSize)
                    let center: CGPoint = CGPoint(x: basePoint.x + xDegreeInterval * CGFloat(i + 1), y: basePoint.y + dataItem.degreeTipMarginVertical + size.height / 2)
                    let label: UILabel = self.getXAxesDegreeTipLabel(tipText: "\(CGFloat(i + 1) * dataItem.xInterval)", center: center, size: size, fontSize: dataItem.degreeTipFontSize)
                    dataItem.targetView.addSubview(label)
                }
            }
            ...
            if (dataItem.yAxesDegreeTexts != nil) {
                for i in 0..<dataItem.yAxesDegreeTexts!.count {
                    ...
                    let label: UILabel = self.getYAxesDegreeTipLabel(tipText: dataItem.yAxesDegreeTexts![i], center: center, size: size, fontSize: dataItem.degreeTipFontSize)
                    dataItem.targetView.addSubview(label)
                }
            } else {
                for i in 0..<yDegreesNum {
                    let size: CGSize = CGSize(width: dataItem.xAxesLeftMargin - dataItem.degreeTipMarginHorizon * 2, height: dataItem.degreeTipFontSize)
                    let center: CGPoint = CGPoint(x: dataItem.xAxesLeftMargin / 2, y: basePoint.y - yDegreeInterval * CGFloat(i + 1))
                    let label: UILabel = self.getYAxesDegreeTipLabel(tipText: "\(CGFloat(i + 1) * dataItem.yInterval)", center: center, size: size, fontSize: dataItem.degreeTipFontSize)
                    dataItem.targetView.addSubview(label)
                }
            }
        }
