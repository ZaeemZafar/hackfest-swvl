//
//  ValidationWrapper.swift
//  ZENFoods
//
//  Created by Abdul Baseer Khan on 30/01/2018.
...
class Validator: NSObject, UITextFieldDelegate {
    ...
    func add(textField: JFTextField, rules: [Validations], showPasswordError: Bool = false, onChange: EmptyEvent? = nil, onReturnEvent: SuccessEvent? = nil) {
    ...
    func removeRules(textField: JFTextField) {
    ...
    func addRules(textField: JFTextField, rules: [Validations]) {
        for validationObject in validationObjects {
            if textField == validationObject.textField {
    ...
    @objc func editingBegin(_ textField: JFTextField) {
    ...
    func getSelectedTextField() -> JFTextField {
    ...
    /*func formatCreditCard(string: String) -> (String, UIImage) {
        var newString = string.replacingOccurrences(of: "/", with: "")
        var gaps: Int = (newString.characters.count + 3) / 4 - 1
        let image = StripeHelper.getCardImage(number: newString)
        while gaps > 0 {
            newString.insert("/", at: newString.index(newString.startIndex, offsetBy: gaps * 4) )
            gaps -= 1
        }
        
        return (newString, image)
    }*/
...
class ValidationObject {
    ...
    init(textField txt: JFTextField, rules rul: [Validations], showErr: Bool = false, onReturn: SuccessEvent? = nil, onChange: EmptyEvent? = nil) {
        textField = txt
        rules = rul
